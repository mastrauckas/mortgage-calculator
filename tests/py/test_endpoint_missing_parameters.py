...
class TestEndpointMissingParameters:
    ...
    def test_missing_all_parameters(self, client):
        ...
        assert validation_error.get('error_message') == 'Parameter \'' + parameter + '\' missing from request.'
    ...
    def test_missing_installments_parameter(self, client):
        response = client.get('/api/v1.0/mortgage?rate=3.12&principal=398483&payment=4040.35')
        assert response.status_code == 422
        assert response.content_type == 'application/json'
        assert isinstance(response.json, dict)
    ...
    def test_missing_principal_parameter(self, client):
        response = client.get('/api/v1.0/mortgage?rate=3.12&installments=10&payment=4040.35')
        assert response.status_code == 422
        assert response.content_type == 'application/json'
        assert isinstance(response.json, dict)
        assert len(response.json['validation_errors']) == 1
    ...
    def test_missing_rate_parameter(self, client):
        response = client.get('/api/v1.0/mortgage?installments=10&principal=398483&payment=4040.35')
        assert response.status_code == 422
        assert response.content_type == 'application/json'
        assert isinstance(response.json, dict)
        assert len(response.json['validation_errors']) == 1
    ...
    def test_missing_payment_parameter(self, client):
        response = client.get('/api/v1.0/mortgage?rate=3.12&installments=10&principal=398483')
        assert response.status_code == 422
        assert response.content_type == 'application/json'
        assert isinstance(response.json, dict)
        assert len(response.json['validation_errors']) == 1
